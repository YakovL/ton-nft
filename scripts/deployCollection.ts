...
export async function run(provider: NetworkProvider) {
    ...
    const collectionConfig: NftCollectionConfig = {
        ...
        collectionContent: nftContentToCell({ uri: overallConfig.collectionContentUri }),
    ...
    const estimatedDeployFee = toNano('0.0002'); // failed for 0.0001
    await collection.sendDeploy(provider.sender(), estimatedDeployFee);
